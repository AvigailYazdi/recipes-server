import {
  ...
  getMaxPrepTimeService,
  getMyRecipeRatingService,
  getRecipeByIdService,
  getRecipeCommentsService,
  getRecipeRatingService,
  rateRecipeService,
...
export const addRecipeController = async (req, res) => {
  try {
    ...
    return res
      .status(400)
      .json({ message: "Error in adding recipe", error: e.message });
  }
};
...
export const getAllRecipesController = async (req, res) => {
  try {
    ...
    if (tag) {
      //can be more than one tag
      const tags = Array.isArray(tag) ? tag : [tag];
      filter.tags = { $in: tags }; // $in- not all tags
    }
    if (q) {
      //for search in title or description
      filter.$or = [
    ...
    return res
      .status(400)
      .json({ message: "Error in getting all recipes", error: e.message });
  }
};
...
export const getRecipeByIdController = async (req, res) => {
  try {
    ...
    return res
      .status(400)
      .json({ message: "Error in getting recipe", error: e.message });
  }
};
...
export const addToFavoriteController = async (req, res) => {
  try {
    ...
    return res
      .status(400)
      .json({ message: "Error in adding to favorites", error: e.message });
  }
};

export const removeFromFavoriteController = async (req, res) => {
  try {
    const favorites = await removeFromFavoriteService(
      req.user.sub,
      req.params.id,
    );
    res.status(200).json(favorites);
    ...
    return res
      .status(400)
      .json({ message: "Error in removing from favorites", error: e.message });
  }
};
...
export const deleteRecipeController = async (req, res) => {
  try {
    ...
    return res
      .status(400)
      .json({ message: "Error in deleting recipe", error: e.message });
  }
};
...
export const updateRecipeController = async (req, res) => {
  try {
    ...
    return res
      .status(400)
      .json({ message: "Error in updating recipe", error: e.message });
  }
};

export const rateRecipeController = async (req, res) => {
  try {
    const rating = await rateRecipeService(
      req.user.sub,
      req.params.id,
      req.body.value,
    );
    ...
    return res
      .status(400)
      .json({ message: "Error in rating recipe", error: e.message });
  }
};
...
export const getRecipeRatingController = async (req, res) => {
  try {
    ...
    return res
      .status(400)
      .json({ message: "Error in getting recipe's rating", error: e.message });
  }
};

export const getMyRecipeRatingController = async (req, res) => {
  try {
    const result = await getMyRecipeRatingService(req.user.sub, req.params.id);
    res.status(200).json(result);
  } catch (e) {
    return res.status(400).json({
      message: "Error in getting my recipe rating",
      error: e.message,
    });
  }
};
...
export const getRecipeCommentsController = async (req, res) => {
  try {
    ...
    res.status(400).json({
      message: "Error in getting recipe's comments",
      error: e.message,
    });
  }
};

export const getMaxPrepTimeController = async (req, res) => {
  try {
    const maxPrepTime = await getMaxPrepTimeService();
    res.json({ maxPrepTime });
  } catch (e) {
    res.status(400).json({
      message: "Error in getting max prep-time",
      error: e.message,
    });
  }
};
