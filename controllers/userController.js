...
export const registerUserController = async (req, res) => {
    try {
        ...
        res.status(201).json({ message: "Register successfully", user: result.user, token: result.token });
    }
    catch (e) {
...
export const loginUserController = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: "Login successfully", user: result.user, token: result.token });
    }
    catch (e) {
...
export const getUserFavoritesController = async (req, res) => {
    try {
        ...
        res.status(400).json({ message: "Error in getting user's favorites", error: e.message });
    }
}
...
export const getAllUsersController = async (req, res) => {
    try {
        ...
        res.status(400).json({ message: "Error in getting all users", error: e.message });
    }
}
